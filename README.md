...
- **A create could claim another crew member's code.** Checking only
  `uid == request.auth.uid` allowed a completion to be filed under any
  `crewCode` — the field the dashboard reads. Creates now
...
| FE07 | `admin` | Everything the commander can |
...
Until then FE07 is an ordinary astronaut and Mission Control will not appear for them.
...
crew dashboard. It has no Mission Control and no Add task controls, and it can
write nothing at all.

    https://teja1995.github.io/AATC_Mission/display.html

**Never sign the Pi in as a crew member.** A crew login puts every admin
control on a wall in a shared space. Create a dedicated account instead:
...
completions, and refuse it every write — `filedBySelf()` excludes it. Someone
walking past the screen cannot mark a test done.
